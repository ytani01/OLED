    ...
    def Render(self, on):
        ...
        for i in range(0, len(arrow), 1):
            ...
            for j in range(0, 3, 1):
                ...
                line >>= 1
        for i in range(0, len(self.Bullets), 1):
...
class Army:
    def __init__(self, device):
        ...
        self.Invaded = False
        self.Invaders = []
        for i in range(0, ARMY_SIZE_ROWS, 1):
            row = []
            for j in range(0, ARMY_SIZE_COLS, 1):
                row.append(Invader(device, 4 + (j * 12) , 30 + (j * 12), 4 + (j * 12), 14 + (i * 12)))
            self.Invaders.append(row)
        return

    def Render(self, on):
        for i in range(0, len(self.Invaders), 1):
            for j in range(0, len(self.Invaders[i]), 1):
...
def AiLogicShoot(army, plyr):
    for i in range(0, len(army.Invaders), 1):
        for j in range(0, len(army.Invaders[i]), 1):
